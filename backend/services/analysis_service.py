...
class AnalysisService:
    ...
    async def _generate_embeddings(self, info: Dict[str, Any], listing: ListingDocument) -> List[float]:
        ...
        for key, value in sorted(info.items()):
            if key not in [
                ...
                if isinstance(value, (list, tuple)):
                    ...
                    other_parts.append(f"{key}: {', '.join(f'{k}={v}' for k, v in sorted(value.items()))}")
                else:
                    other_parts.append(f"{key}: {value}")

        # Combine all parts, with core information at the start
        text = " | ".join(text_parts + other_parts)
        self.logger.debug(f"Generated embedding text for listing {listing.original_id}: {text[:200]}...")
        ...
        for i in range(0, len(listings), batch_size):
            ...
            try:
                ...
                for j, listing in enumerate(batch):
                    # If we only have one listing, the data is not inside a list
                    ...
                    try:
                        ...
                        embeddings = await self._generate_embeddings(info, listing)
