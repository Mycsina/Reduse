...
class AnalysisService:
    ...
    async def _generate_listing_embeddings(
        self, info: Dict[str, Any], listing: ListingDocument
    ) -> List[float]:
    ...
    async def analyze_batch(
        ...
        for i in range(0, len(listings), batch_size):
            ...
            self.logger.debug(
                f"Processing batch {batch_num} with {len(batch)} listings"
            )
            ...
            for listing in batch:
                try:
                    ...
                    self.logger.debug(
                        f"Querying model for listing {listing.original_id}"
                    )
                    analysis = await self.ai_model.query(prompt, input=input_text)
                    ...
                    self.logger.warning(
                        f"Rate limit exceeded, waiting {e.retry_after} seconds"
                    )
    ...
    async def get_pending_listings(self) -> List[ListingDocument]:
        ...
        return await ListingDocument.find(
            {"$or": [{"analysis_status": AnalysisStatus.PENDING}]}
        ).to_list()
    ...
    async def bulk_create_analyses(self, analyses: List[AnalyzedListingDocument]):
        ...
        [
            self.logger.debug(f"Creating {analysed.original_listing_id} in bulk")
