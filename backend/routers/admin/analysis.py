...
async def retry_failed_analyses(background_tasks: BackgroundTasks, _: str = Depends(verify_api_key)):
    ...
    return AnalysisStatusResponse(
        message=f"Retrying {status.failed} failed analyses",
        **status.model_dump()
    )
...
async def start_analysis(background_tasks: BackgroundTasks, _: str = Depends(verify_api_key)):
    ...
    if status.pending == 0:
        return AnalysisStatusResponse(
            message="No pending analyses to start",
            **status.model_dump()
        )

    background_tasks.add_task(analysis.analyze_new_listings)
    return AnalysisStatusResponse(
        message=f"Started processing {status.pending + status.failed} unanalyzed listings",
        **status.model_dump()
    )
...
async def resume_analysis(background_tasks: BackgroundTasks, _: str = Depends(verify_api_key)):
    ...
    if status.in_progress == 0:
        return AnalysisStatusResponse(
            message="No in-progress analyses to resume",
            **status.model_dump()
        )

    background_tasks.add_task(analysis.resume_analysis)
    return AnalysisStatusResponse(
        message=f"Resuming {status.in_progress} in-progress analyses",
        **status.model_dump()
    )
...
async def cancel_analysis(_: str = Depends(verify_api_key)):
    ...
    return CancelAnalysisResponse(
        message=f"Cancelled {cancelled} in-progress analyses",
        cancelled=cancelled
    )
