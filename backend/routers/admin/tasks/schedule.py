...
from backend.tasks.scheduler import TaskConfig, scheduler
from backend.tasks.task_registry import JobLogFilter, JobStatus, registry
...
async def schedule_scraping(config: ScrapeSchedule, _: str = Depends(verify_security)):
...
async def schedule_analysis(config: AnalysisSchedule, _: str = Depends(verify_security)):
...
async def schedule_maintenance(config: MaintenanceSchedule, _: str = Depends(verify_security)):
...
async def list_jobs(_: str = Depends(verify_security)):
...
async def pause_job(job_id: str, _: str = Depends(verify_security)):
...
async def resume_job(job_id: str, _: str = Depends(verify_security)):
    """Resume a paused job."""
    try:
        await scheduler.init()
...
async def delete_job(job_id: str, _: str = Depends(verify_security)):
...
async def schedule_function(request: CreateTaskRequest, _: str = Depends(verify_security)):
...
async def run_function(request: RunFunctionRequest, _: str = Depends(verify_security)):
...
async def get_job_status(job_id: str, _: str = Depends(verify_security)):
