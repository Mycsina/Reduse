...
from llama_index.core import Embedding
...
class CompositeProvider(BaseProvider):
    """Provider that combines multiple AI providers with LangChain/LlamaIndex compatibility.
    ...
    def __init__(self):
        ...
        self.supports_batch = self.groq.supports_batch  # Inherit batch support from Groq
    ...
    async def process_batch(
        ...
        results = await self.groq.process_batch(requests, model)
    ...
    async def get_batch_status(self, batch_id: str) -> Dict[str, Any]:
        ...
        batch_doc = await self.groq.get_batch_status(batch_id)
        result = {
    ...
    async def cancel_batch(self, batch_id: str) -> None:
        ...
        await self.groq.cancel_batch(batch_id)
    ...
    async def list_batches(
        ...
        batch_docs = await self.groq.list_batches(status=status, limit=limit, skip=skip)
        results = [
            {
    ...
    def as_langchain_runnable(self) -> Runnable:
        """Get a LangChain-compatible runnable.
        
        Returns:
            Runnable: LangChain runnable for this provider
        """
        return RunnableLambda(lambda x: self.generate_json(**x))

    def as_llamaindex_embedding(self) -> Embedding:
        """Get a LlamaIndex-compatible embedding model.
        
        Returns:
            Embedding: LlamaIndex embedding model
        """
        return Embedding(
            embed_fn=self.get_embeddings,
            embed_batch_size=50,
            embed_dim=self._dimensions
        )
