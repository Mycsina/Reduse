...
import { FilterCondition, FilterGroup } from "@/lib/types";
...
function ConditionComponent({
  ...
  const { data: fields, isLoading } = useQuery({
    queryKey: ["fields"],
    queryFn: () => apiClient.getAvailableFields(),
    staleTime: Infinity,
    gcTime: Infinity,
  });
...
export default function AdvancedFilter({
  ...
  const [filter, setFilter] = useState<FilterGroup>(() => {
    ...
    if (advancedParam) {
      try {
        return JSON.parse(advancedParam) as FilterGroup;
      } catch (e) {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      if (e.key === "Enter") {
        e.preventDefault();
        onFilterChange(filter.conditions.length > 0 ? filter : null);
      }
    },
